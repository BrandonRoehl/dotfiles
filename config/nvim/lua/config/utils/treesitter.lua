...
function M.check()
	...
	---@class utils.treesitter.Health: table<string,boolean>
	local ret = {
...
function M:build(cb)
	self.ensure_treesitter_cli(function(_, err)
		...
		if ok then
			...
			LazyUtils.error(lines, { title = "Treesitter" })
		end
	end)
end
...
function M.ensure_treesitter_cli(cb)
	...
	mr.refresh(function()
		...
		if not p:is_installed() then
			LazyUtils.info("Installing `tree-sitter-cli` with `mason.nvim`...")
			p:install(
				nil,
				vim.schedule_wrap(function(success)
					if success then
						LazyUtils.info("Installed `tree-sitter-cli` with `mason.nvim`.")
						cb(true)
					else
